...
2.  **Create the Database Tables via SQL:**
    ...
    * Paste the entire script below and click **"Run"**. This will create all the necessary tables.
    ...
    CREATE TABLE public.user_preferences (
        id uuid NOT NULL,
        default_calories smallint NOT NULL,
        default_protein smallint NOT NULL,
        default_carbs smallint NOT NULL,
        default_fats smallint NOT NULL,
    ...
    CREATE TABLE public.entries (
        ...
        goal_calories smallint NOT NULL,
        goal_protein smallint NOT NULL,
        goal_carbs smallint NOT NULL,
        goal_fats smallint NOT NULL,
    ...
    CREATE TABLE public.recipes (
        ...
        calories_per_serving smallint NOT NULL,
        protein_per_serving smallint NOT NULL,
        carbs_per_serving smallint NOT NULL,
        fats_per_serving smallint NOT NULL,
...
4.  **Add Performance Indexes:**
    * After the tables and trigger are created, run the following script in the **SQL Editor**.
    * These indexes will significantly speed up common operations like loading the daily log and browsing recipes.

    ```sql
    -- For faster daily log lookups
    CREATE INDEX idx_entries_user_date ON public.entries (user_id, entry_date);

    -- For faster "My Recipes" lookups
    CREATE INDEX idx_recipes_user_id ON public.recipes (user_id);

    -- For faster "Public Recipes" lookups
...
* **Meal Templates:** Implement a feature to save frequently eaten meals for quick one-click entry. --DONE (need to add an update recipe function)
...
* **Optimization and User Limitations:** Implement storage and performace optimizations. Limit recipes and entries from users.